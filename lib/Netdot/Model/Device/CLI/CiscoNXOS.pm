package Netdot::Model::Device::CLI::CiscoNXOS;
...
Netdot::Model::Device::CLI::CiscoNXOS - Cisco NXOS Class
...
sub _get_arp_from_cli {
    ...
    my @output = $self->_cli_cmd(%$args, host=>$host, cmd=>'show ip arp vrf all');
...
sub _get_v6_nd_from_cli {
    ...
    my @output = $self->_cli_cmd(%$args, host=>$host, cmd=>'show ipv6 neighbor vrf all');
    shift @output; # Ignore header line
    my %cache;
    foreach my $line ( @output ) {
	my ($ip, $mac, $iname);
...
sub _get_fwt_from_cli {
	...
	if ( $line =~ /^[*\s\+]\s+\S+\s+($CISCO_MAC)\s+dynamic\s+\S+\s+\S+\s+\S+\s+(\S+)\s*$/o ) {
	    ...
	    $logger->debug(sub{"Device::CLI::CiscoNXO::_get_fwt_from_cli: ".
	...
	unless ( $intid ) {
	    $logger->warn("Device::CLI::CiscoNXOS::_get_fwt_from_cli: $host: ".
	...
	if ( my $e = $@ ){
	    $logger->debug(sub{"Device::CLI::CiscoNXOS::_get_fwt_from_cli: ".
				   "$host: Invalid MAC: $e" });
	    next;
	}	
	# Store in hash
